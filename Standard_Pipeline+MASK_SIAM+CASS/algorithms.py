...
from modeling.losses import DahLoss, GDRNetLoss_Integrated
...
class CASS_GDRNet(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=cfg.LEARNING_RATE, weight_decay=0.0001)
        self.K = 1024
        proj_dim = 1024
    ...
    def update(self, minibatch):
        ...
        img_new, _ = self.fundusAug['post_aug1'](image.clone(), mask.clone())
        img_new = self.fundusAug['post_aug2'](img_new)
        img_ori = self.fundusAug['post_aug2'](image.clone())
        res_clean = self.network(x_cnn=img_new, x_vit=img_new)
        x_split = self._local_split(img_ori)
        ...
        loss_total = loss_main + 0.5 * loss_fastmoco
        loss_total.backward()
        self.optimizer.step()
    ...
    def validate(self, val_loader, test_loader, writer):
        ...
        test_auc = -1
        if self.epoch <= self.cfg.EPOCHS:
            val_metrics, val_loss = algorithm_validate(self, val_loader, writer, self.epoch, 'val')
            val_auc = val_metrics['auc']
            test_metrics, test_loss = algorithm_validate(self, test_loader, writer, self.epoch, 'test')
            test_auc = test_metrics['auc']
            if self.epoch == self.cfg.EPOCHS:
                self.epoch += 1
        else:
            test_metrics, test_loss = algorithm_validate(self, test_loader, writer, self.cfg.EPOCHS + self.cfg.VAL_EPOCH, 'test')
            test_auc = test_metrics['auc']
            logging.info(f'Final Test Result (CNN) >> AUC: {test_auc:.4f}')
        return val_auc, test_auc

    def predict(self, x):
        return self.network(x_cnn=x)

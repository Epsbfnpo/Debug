...
class GDRNetLoss_Integrated(nn.Module):
    def __init__(self, training_domains, beta):
        ...
        self.domain_num_dict = {'MESSIDOR': 1396, 'IDRID': 413, 'APTOS': 3662, 'DEEPDR': 1000}
        self.label_num_dict = {'MESSIDOR': [824, 219, 211, 60, 28, 48, 6, 0], 'IDRID': [134, 20, 136, 74, 49, 0, 0, 0], 'APTOS': [1805, 370, 999, 193, 295, 0, 0, 0], 'DEEPDR': [410, 89, 276, 76, 149, 0, 0, 0]}
    ...
    def get_dcr_weights(self, labels, domains):
        ...
        combined_weight = combined_weight / (combined_weight.mean() + eps)
    ...
    def forward(self, output_dict, queue_dict, labels, domains):
        ...
        loss_cass_raw = F.mse_loss(probs_cnn, probs_vit, reduction='none').mean(dim=1)
        loss_cass = (loss_cass_raw * dcr_weight).mean()
        prototypes = queue_dict['prototypes']
        proj_cnn_norm = F.normalize(proj_cnn, dim=1)
        proj_vit_norm = F.normalize(proj_vit, dim=1)
        proto_norm = F.normalize(prototypes, dim=1)
        target_protos = proto_norm[labels]
        loss_queue_cnn = 1.0 - (proj_cnn_norm * target_protos).sum(dim=1).mean()
        loss_queue_vit = 1.0 - (proj_vit_norm * target_protos).sum(dim=1).mean()
        lambda_sup = 1.0
        lambda_cass = 2.0
        lambda_queue = 0.5
        loss_total = lambda_sup * (loss_sup_cnn + loss_sup_vit) + lambda_cass * loss_cass + lambda_queue * (loss_queue_cnn + loss_queue_vit)
        loss_dict = {"loss": loss_total.item(), "sup_cnn": loss_sup_cnn.item(), "sup_vit": loss_sup_vit.item(), "cass": loss_cass.item(), "queue": (loss_queue_cnn + loss_queue_vit).item() / 2}

...
def main():
    ...
    has_dual_best = (best_performance_cnn > 0 or best_performance_vit > 0)
    if has_dual_best:
        branches_to_test = []
        if best_performance_cnn > 0: branches_to_test.append('cnn')
        if best_performance_vit > 0: branches_to_test.append('vit')
        for branch in branches_to_test:
            ...
            test_metrics, test_loss = algorithm_validate(algorithm, test_loader, writer, cfg.EPOCHS, f'test_{branch}')
            ...
            if args.local_rank in [-1, 0]:
                logging.info(f"🚀 Final Test Results [{branch.upper()}] - AUC: {target_auc:.4f}, Acc: {target_acc:.4f}, F1: {target_f1:.4f}")
                with open(os.path.join(log_path, f'done_{branch}'), 'w') as f:
                    f.write(f'done, best_val={best_performance_cnn if branch == "cnn" else best_performance_vit:.4f}, test_auc={target_auc:.4f}')
    else:
        if args.local_rank in [-1, 0]:

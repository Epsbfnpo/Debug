...
def main():
    ...
    for i in iterator:
        ...
        if epoch % cfg.VAL_EPOCH == 0:
            # [Fix 2] 死锁修复：验证过程必须由所有 Rank 参与 (包含集合通信)
            if args.local_rank in [-1, 0]:
                logging.info("Starting validation...")

            # 所有进程都必须执行 validate，内部会自动处理同步
            val_metrics, _ = algorithm.validate(val_loader, None, writer)
            val_auc = val_metrics['auc']

            # 只有 Rank 0 负责打印日志和保存最佳模型
            if args.local_rank in [-1, 0]:
    ...
    # 接收 Test 指标字典
    # 同样需要注意：如果在 DDP 模式下 validate 有集合通信，这里最好也全员参与
    # 但原代码结构中 renew_model 似乎只在 rank 0 有意义，或者 algorithm.renew_model 内部处理了加载
    # 为保险起见，这里保持所有 rank 参与 validate

...
def resnet50(pretrained=True, **kwargs):
    ...
    """
    # 1. 构建你的自定义 ResNet 模型结构 (保持这行不变，确保兼容性)
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)

    # 2. 修改后的权重加载逻辑
    if pretrained and cfg.MODEL.PRETRAINED:
        # 获取 defaults.py 中定义的本地路径
        ...
        if not os.path.exists(path):
            ...
            print(f"并保存到: {path}")
            exit(1)  # 直接退出，防止用随机权重继续训练

        try:
            # 加载本地 .pth 文件
            state_dict = torch.load(path, map_location='cpu')

            # 这里的 strict=False 非常重要！
            # 因为 ResNet 预训练权重通常包含 fc 层 (1000类)，
            # 而你的任务可能是 5 类，或者是作为特征提取器使用，
            # strict=False 允许忽略不匹配的层 (如 fc)。
            msg = model.load_state_dict(state_dict, strict=False)

            print(f"[ResNet] ✅ 权重加载成功！")
            # 打印未匹配的键，通常应该是 'fc.weight', 'fc.bias' 等
        ...
        # 如果 pretrained=True 但配置里没开 (理论上不该发生)，尝试原版逻辑或报错

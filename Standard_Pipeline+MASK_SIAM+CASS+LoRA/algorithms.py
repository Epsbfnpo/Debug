...
class CASS_GDRNet(Algorithm):
    ...
    def update(self, minibatch):
        ...
        img_strong = self.fundusAug['post_aug2'](img_strong)
        img_weak = self.fundusAug['post_aug2'](image.clone())
        x_cnn_input = img_strong
        x_vit_input = img_weak
        res_clean = self.network(x_cnn=x_cnn_input, x_vit=x_vit_input)
        x_split = self._local_split(img_weak)
        feat_split = network_inner.cnn(x_split)
        if len(feat_split.shape) > 2: feat_split = feat_split.view(feat_split.size(0), -1)
        ...
        self.dequeue_and_enqueue(res_clean['proj_vit'].detach(), label)
        self.compute_prototypes_from_queue()
        queue_dict = {'prototypes': self.prototypes}
        loss_main, loss_dict = self.criterion(res_clean, queue_dict, label, domain)
        ...
        feat_cnn_norm = F.normalize(res_clean['proj_cnn'], dim=1)
        feat_vit_norm = F.normalize(res_clean['proj_vit'], dim=1)
        features_multi = torch.stack([feat_cnn_norm, feat_vit_norm], dim=1)
        loss_supcon = self.supcon_loss(features_multi, label)
        loss_total = loss_main + 0.5 * loss_fastmoco + 1.0 * loss_supcon
        loss_total.backward()
        self.optimizer.step()
    ...
    def validate(self, val_loader, test_loader, writer):
        ...
        if self.epoch <= self.cfg.EPOCHS:
            ...
            val_auc_cnn = val_metrics['auc']
            test_metrics, _ = algorithm_validate(self, test_loader, writer, self.epoch, 'test')
            test_auc_cnn = test_metrics['auc']
        self.eval_branch = 'vit'
        val_metrics_vit, _ = algorithm_validate(self, val_loader, None, self.epoch, 'val_vit')
        val_auc_vit = val_metrics_vit['auc']
        test_metrics_vit, _ = algorithm_validate(self, test_loader, None, self.epoch, 'test_vit')
        test_auc_vit = test_metrics_vit['auc']
        self.eval_branch = 'cnn'
        if self.epoch == self.cfg.EPOCHS:
            self.epoch += 1
        if self.epoch > self.cfg.EPOCHS:
            logging.info(f'Final Test Result (CNN) >> AUC: {test_auc_cnn:.4f}')
            logging.info(f'Final Test Result (ViT) >> AUC: {test_auc_vit:.4f}')
            logging.info(f'Comparison (Val)  >> CNN: {val_auc_cnn:.4f} vs ViT: {val_auc_vit:.4f}')
        return val_auc_cnn, test_auc_cnn

    def predict(self, x):
        res = self.network(x_cnn=x)
        if self.eval_branch == 'cnn':
            return res['logits_cnn']
        elif self.eval_branch == 'vit':
            return res['logits_vit']
        else:
            raise ValueError(f"Unknown eval_branch: {self.eval_branch}")

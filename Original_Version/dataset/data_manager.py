...
def get_transform(cfg):
    ...
    tra_train = v2.Compose([lambda img: square_tight_crop(img, target_size=re_size), v2.RandomHorizontalFlip(p=0.5), v2.RandomVerticalFlip(p=0.5), v2.RandomRotation(45), v2.ColorJitter(0.3, 0.3, 0.3, 0.05), v2.ToTensor(), normalize,])
    tra_test = v2.Compose([lambda img: square_tight_crop(img, target_size=re_size), v2.ToTensor(), normalize,])
...
def get_pre_FundusAug(cfg):
    ...
    normalize = get_normalize()
    weak_transforms = v2.Compose([lambda img: square_tight_crop(img, target_size=512), v2.Resize((512, 512), antialias=True), v2.ToTensor(), normalize,])
    vit_train_transforms = v2.Compose([lambda img: square_tight_crop(img, target_size=512), v2.Resize((512, 512), antialias=True), v2.RandomHorizontalFlip(p=0.5), v2.RandomVerticalFlip(p=0.5), v2.RandomRotation(45), v2.ColorJitter(brightness=jitter_b, contrast=jitter_c, saturation=jitter_s, hue=jitter_h), v2.ToTensor(), normalize,])

...
class CASS_GDRNet(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        head_modules = [self.network.projector_cnn, self.network.projector_vit, self.network.predictor_cnn, self.network.predictor_vit, self.network.classifier_cnn, self.network.classifier_vit, self.network.dual_stream_neck]
        head_params = []
        ...
        self.backbone_params = [p for p in self.network.parameters() if p.requires_grad and id(p) not in head_param_ids]
        self.optimizer = AdamW([{'params': self.backbone_params, 'lr': self.base_lr_backbone}, {'params': self.head_params, 'lr': self.base_lr_head}], weight_decay=5e-2)
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=max(1, self.cfg.EPOCHS - self.warmup_epochs), eta_min=1e-6)
        ...
        self.weak_transforms_cnn = v2.Compose([v2.Resize((512, 512), antialias=True), v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
    ...
    def _update_class_prototypes(self, feat_cnn, feat_vit, labels):
        ...
        for cls in unique_labels:
            ...
            if not self.class_proto_initialized[cls_idx]:
                ...
                self.class_proto_cnn[cls_idx] = F.normalize(mom * self.class_proto_cnn[cls_idx] + (1.0 - mom) * batch_center_cnn, dim=0, eps=1e-6,)
                self.class_proto_vit[cls_idx] = F.normalize(mom * self.class_proto_vit[cls_idx] + (1.0 - mom) * batch_center_vit, dim=0, eps=1e-6,)
    ...
    def update(self, minibatch):
        ...
        res_clean_fp32 = {'proj_cnn': res_combined['proj_cnn'].float(), 'proj_vit': res_combined['proj_vit'].float(), 'pred_cnn': res_combined['pred_cnn'].float(), 'pred_vit': res_combined['pred_vit'].float(), 'logits_cnn': res_combined['logits_cnn'].float(), 'logits_vit': res_combined['logits_vit'].float(), 'tia_cls': res_combined['tia_cls'].float(), 'spatial_tokens': res_combined['spatial_tokens'].float(), 'feat_vit': res_combined['feat_vit'].float(),}
        with torch.no_grad():

...
class ERM(Algorithm):
    ...
    def __init__(self, num_classes, cfg):
        ...
        # ------------------- [修改开始] -------------------
        # 针对 FPT+ (Transformer 架构) 自动切换为 AdamW 优化器
        # 也可以在 Config 中增加一个字段 cfg.SOLVER.OPTIMIZER 来控制，这里用 Backbone 名称判断更直接
        if 'FPT' in cfg.BACKBONE:
            # AdamW 通常需要比 SGD 更小的学习率 (e.g., 1e-4 ~ 5e-5)
            # 且 AdamW 自带 Weight Decay 处理机制

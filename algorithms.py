...
from modeling.nets import LossValley, AveragedModel, DINOv3_FD_Net, MINE
...
class CASS_GDRNet(Algorithm):
    def __init__(self, num_classes, cfg):
        ...
        self.mine_net = MINE(in_dim=256 * 2, hidden=256)
        self.lambda_ortho = getattr(cfg.GDRNET, 'LAMBDA_ORTHO', 0.005)
        self.lambda_reg = getattr(cfg.GDRNET, 'LAMBDA_REG', 0.2)
        self.num_classes = num_classes

        main_lr = getattr(cfg.GDRNET, 'MAIN_LR', 5e-5)
        main_weight_decay = getattr(cfg.GDRNET, 'MAIN_WEIGHT_DECAY', 5e-2)
        mine_lr = getattr(cfg.GDRNET, 'MINE_LR', 1e-4)
    ...
    def update(self, minibatch):
        ...
        logits_t, logits_i, f_t, f_i = self.network(image)

        self.optimizer_mine.zero_grad()
        mi_pos = self.mine_net.estimate_mi(f_t.detach(), f_i.detach())
        loss_mine = -mi_pos
        loss_mine.backward()
        self.optimizer_mine.step()

        self.optimizer_main.zero_grad()
        loss_cls = F.cross_entropy(logits_t, label)
        loss_ortho = self.mine_net.estimate_mi(f_t, f_i)
        log_probs_i = F.log_softmax(logits_i, dim=1)
        uniform_targets = torch.ones_like(logits_i) / self.num_classes
        loss_reg = F.kl_div(log_probs_i, uniform_targets, reduction='batchmean')
        loss_total = loss_cls + self.lambda_ortho * loss_ortho + self.lambda_reg * loss_reg
        loss_total.backward()
    ...
    def validate(self, val_loader, test_loader, writer):
        ...
        test_auc = -1
        if self.epoch <= self.cfg.EPOCHS:
            val_metrics, _ = algorithm_validate(self, val_loader, writer, self.epoch, 'val')
            test_metrics, _ = algorithm_validate(self, test_loader, writer, self.epoch, 'test')
            val_auc = val_metrics['auc']
            test_auc = test_metrics['auc']
            if self.epoch == self.cfg.EPOCHS:
                self.epoch += 1
        else:
            test_metrics, _ = algorithm_validate(self, test_loader, writer, self.cfg.EPOCHS + self.cfg.VAL_EPOCH, 'test')
            test_auc = test_metrics['auc']
            logging.info('Best performance on test domain(s): {}'.format(test_auc))
        return val_auc, test_auc
    ...
    def save_model(self, log_path, source='best'):
        ...
        if rank == 0:
            ...
            torch.save(model_to_save.state_dict(), os.path.join(log_path, 'best_model.pth'))
            torch.save(self.mine_net.state_dict(), os.path.join(log_path, 'best_mine.pth'))

    def renew_model(self, log_path, source='best'):
        model_path = os.path.join(log_path, 'best_model.pth')
        mine_path = os.path.join(log_path, 'best_mine.pth')
        if os.path.exists(model_path):
            ...
            logging.info('✅ Model renewed from best_model.pth')
        else:
            logging.warning('⚠️ Could not find best_model.pth, skipping load.')
        if os.path.exists(mine_path):
            self.mine_net.load_state_dict(torch.load(mine_path, map_location='cpu'))

...
class GDRNet(ERM):
    def __init__(self, num_classes, cfg):
        ...
        # [AMP Optimization] 初始化 GradScaler
        print(">> [Optimization] Initializing AMP GradScaler for Mixed Precision Training...")
        self.scaler = torch.cuda.amp.GradScaler()

    # ... (patchify, unpatchify, random_masking, _local_split, dequeue, sample_target, update_ema, img_process, ortho_loss 等辅助函数保持不变，此处省略以节省篇幅) ...
    # 必须保留上述辅助函数！复制时请确保包含它们。
    ...
    # [Update] 整合了 AMP 和 梯度累积 的 update 函数
    def update(self, minibatch, step=None, accum_iter=1):
        # 兼容性处理：如果未传入 step，则默认不进行累积控制（fallback）
        if step is None:
        ...
        # 2. 如果是累积周期的第一步，清空梯度
        # 注意：在 Accumulation 模式下，不要每步都 zero_grad
        ...
        with autocast():
            ...
            # [Accumulation] Loss 缩放
            total_loss = total_loss / accum_iter

        # ------------------------------------------------------------------
        # [AMP End] 退出混合精度上下文，使用 scaler 处理梯度
        # ------------------------------------------------------------------
        ...
        loss_dict_iter['loss'] = total_loss.item() * accum_iter  # 还原 logging 用的 loss 数值

        # Backward (Scaled)
        self.scaler.scale(total_loss).backward()

        # Step (Only on boundary)
        if is_update_step:
            self.scaler.step(self.optimizer)
            self.scaler.update()
            self.optimizer.zero_grad()

            # EMA 和 Queue 更新也应该只在真正更新参数时进行
            self.update_ema_model()
            self.dequeue_and_enqueue(z_new_ema.detach(), label)

        return loss_dict_iter

    # ... predict, save_model, renew_model 保持不变 ...
    def predict(self, x):
        out = self.network(x)

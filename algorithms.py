...
class GDRNet(ERM):
    def __init__(self, num_classes, cfg):
        ...
        self.dino_projector = nn.Sequential(nn.Linear(dino_dim, dim_in, bias=False), nn.BatchNorm1d(dim_in), nn.ReLU(inplace=True), nn.Linear(dim_in, dim_in, bias=False), nn.BatchNorm1d(dim_in), nn.ReLU(inplace=True), nn.Linear(dim_in, feat_dim, bias=False), nn.BatchNorm1d(feat_dim, affine=False))
        ...
        self.optimizer = torch.optim.AdamW([{"params": self.network.parameters(), 'fix_lr': False}, {"params": self.projector.parameters(), 'fix_lr': False}, {"params": self.dino_projector.parameters(), 'fix_lr': False}, {"params": self.predictor.parameters(), 'fix_lr': True},], lr=cfg.LEARNING_RATE, weight_decay=cfg.WEIGHT_DECAY)
        ...
        if self.sdclr_enabled:
            ...
            self.pruning_mask = Mask(self.network.model)

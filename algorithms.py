...
class CASS_GDRNet(Algorithm):
    ...
    def update(self, minibatch):
        ...
        res_clean_fp32 = {'proj_cnn': res_combined['proj_cnn'].float(), 'proj_vit': res_combined['proj_vit'].float(), 'logits_cnn': res_combined['logits_cnn'].float(), 'logits_vit': res_combined['logits_vit'].float(), 'logits_fusion': res_combined['logits_fusion'].float(), 'drts': res_combined['drts'].float(), 'spatial_tokens': res_combined['spatial_tokens'].float(),}
        with torch.no_grad():
            with torch.amp.autocast('cuda'):

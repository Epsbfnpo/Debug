...
class GDRNet(Algorithm):
    ...
    def update(self, minibatch, step=None, accum_iter=None):
        ...
        # =============================================================
        lambda_main = self.cfg.GDRNET.LAMBDA_SUP
        lambda_ortho = self.cfg.GDRNET.LAMBDA_ORTHO
        lambda_spmix = self.cfg.GDRNET.LAMBDA_SPMIX
        lambda_feat_align = self.cfg.GDRNET.LAMBDA_FEAT_ALIGN
        lambda_logit_kd = self.cfg.GDRNET.LAMBDA_LOGIT_KD
        lambda_dino_cls = self.cfg.GDRNET.LAMBDA_DINO_CLS
        lambda_global = self.cfg.GDRNET.LAMBDA_GLOBAL

        total_loss = loss_main * lambda_main + lambda_ortho * loss_ortho + lambda_spmix * loss_spmix + lambda_feat_align * loss_feat_align + lambda_logit_kd * loss_logit_kd + lambda_dino_cls * loss_dino_cls + lambda_global * loss_global

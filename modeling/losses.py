...
class GDRNetLoss_Integrated(nn.Module):
    ...
    def forward(self, output_dict, queue_dict, labels, domains):
        ...
        temperature = 0.1
        logits_proto_cnn = torch.matmul(proj_cnn_norm, proto_norm.T) / temperature
        logits_proto_vit = torch.matmul(proj_vit_norm, proto_norm.T) / temperature
        loss_queue_cnn = (self.SupLoss(logits_proto_cnn, labels) * dcr_weight).mean()
        loss_queue_vit = (self.SupLoss(logits_proto_vit, labels) * dcr_weight).mean()
        lambda_sup = 1.0
        lambda_cass = 0.5
        lambda_queue = 0.5

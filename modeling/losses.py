...
class GDRNetLoss_Integrated(nn.Module):
    def __init__(self, max_iteration, training_domains, beta=0.8, scaling_factor=4, alpha=1.0, temperature=0.07, gamma=1.0):
    ...
    def get_domain_label_prob(self):
        source_domain_num_list = torch.Tensor([self.domain_num_dict[domain] for domain in self.training_domains])
        domain_prob = source_domain_num_list / torch.sum(source_domain_num_list)
        label_num_list = torch.Tensor([self.label_num_dict[domain] for domain in self.training_domains]).sum(dim=0)
        label_prob = label_num_list / torch.sum(label_num_list)
        return domain_prob.cuda(), label_prob.cuda()

    def multinomial_soomthing(self, domain_prob, label_prob, beta):
        domain_prob = torch.pow(domain_prob, beta)
        label_prob = torch.pow(label_prob, beta)
        return domain_prob / torch.sum(domain_prob), label_prob / torch.sum(label_prob)

    def get_weights(self, labels, domains):
        domain_weight = 1 / torch.index_select(self.domain_prob, 0, domains).cuda()
        class_weight = 1 / torch.index_select(self.label_prob, 0, labels).cuda()
        return domain_weight, class_weight
    ...
    def forward(self, output_logits, contrastive_features, labels, domains):
        domain_weight, class_weight = self.get_weights(labels, domains)
        loss_sup = self.SupLoss(output_logits, labels)
        loss_sup = torch.mean(loss_sup * class_weight * domain_weight) / (torch.mean(domain_weight) * torch.mean(class_weight))
        z1, z2, z1_sup, z2_sup, p1, p2, p1_orthmix, p2_orthmix = contrastive_features
        SSL_loss = 0.5 * (D(p1, z2) + D(p2, z1))
        Sup_loss = 0.5 * (D(p1, z2_sup) + D(p2, z1_sup))
        loss_siam_base = 0.5 * SSL_loss + 0.5 * Sup_loss
        loss_fastmoco = 0.0
        if p1_orthmix is not None and len(p1_orthmix) > 0:
            SSL_loss_fm = 0.0
            Sup_loss_fm = 0.0
            for i in range(len(p1_orthmix)):
                p1_mix, p2_mix = p1_orthmix[i], p2_orthmix[i]
                SSL_loss_fm += 0.5 * (D(p1_mix, z2) + D(p2_mix, z1))
                Sup_loss_fm += 0.5 * (D(p1_mix, z2_sup) + D(p2_mix, z1_sup))
            loss_fastmoco = (0.5 * SSL_loss_fm + 0.5 * Sup_loss_fm) / len(p1_orthmix)
        loss_contrast = loss_siam_base + self.gamma * loss_fastmoco
        loss = (1 - self.alpha) * loss_sup + self.alpha * loss_contrast
        return loss, {"loss": loss.item(), "loss_sup": loss_sup.item(), "loss_contrast": loss_contrast.item()}
...
class GDRNet(ERM):
    def __init__(self, num_classes, cfg):
        ...
        self.dense_projector = nn.Sequential(nn.Conv2d(dim_in, dino_dim, kernel_size=1, bias=False), nn.BatchNorm2d(dino_dim), nn.ReLU(inplace=True), nn.Conv2d(dino_dim, dino_dim, kernel_size=1))

        self.optimizer = torch.optim.Adam([{"params": self.network.parameters(), 'fix_lr': False}, {"params": self.projector.parameters(), 'fix_lr': False}, {"params": self.predictor.parameters(), 'fix_lr': True}, {"params": self.tra_projector.parameters(), 'fix_lr': False}, {"params": self.tia_projector.parameters(), 'fix_lr': False}, {"params": self.dino_classifier.parameters(), 'fix_lr': False}, {"params": self.dino_contrast_projector.parameters(), 'fix_lr': False}, {"params": self.dino_predictor.parameters(), 'fix_lr': False}, {"params": self.dense_projector.parameters(), 'fix_lr': False}, ], lr=cfg.LEARNING_RATE, weight_decay=cfg.WEIGHT_DECAY)
        ...
        self.register_buffer("label_queue", -torch.ones(self.K, dtype=torch.long))
        ...
        if hasattr(self.network, 'model') and hasattr(self.network.model, 'layer4'):
            self.network.model.layer4.register_forward_hook(get_hook('layer4'))
            print(">> [GDRNet] Hook registered on SideResNet layer4 for standard AT.")

            """
            ===================================================================
            [MICCAI Time Capsule: Advanced Idea 3 - Pyramid AT]
            ...
            print(">> [Warning] Could not find SideResNet layer4.")
    ...
    def compute_saliency_score(self, attn_map):
        ...
        return (scores / 2.0 + 0.5).to(dtype=torch.bfloat16)
    ...
    def _dequeue_and_enqueue(self, keys, labels):
        ...
        if ptr + batch_size > self.K:
            ...
            self.label_queue[ptr:self.K] = labels[:rem]
            self.queue[0:batch_size - rem] = keys[rem:]
            self.label_queue[0:batch_size - rem] = labels[rem:]
        else:
    ...
    def img_process(self, img_tensor, mask_tensor, fundusAug):
        ...
        return fundusAug['post_aug2'](img_tensor_new), fundusAug['post_aug2'](img_tensor)

    def orthogonality_loss(self, z_tra, z_tia):
        cosine = (F.normalize(z_tra, dim=-1) * F.normalize(z_tia, dim=-1)).sum(dim=-1)
    ...
    def update(self, minibatch, step=None, accum_iter=1):
        ...
        with autocast(dtype=torch.bfloat16):
            ...
            with torch.no_grad():
                dino_outputs = raw_network.frozen_encoder(image_ori, interpolate_pos_encoding=True, output_attentions=True)
                attentions = dino_outputs.attentions[-1]
                raw_attn = attentions[:, :, 0, 1:]

                n_tokens_all = raw_attn.shape[-1]
                side = int(n_tokens_all ** 0.5)
                n_patches = side ** 2

                cls_attn = raw_attn[:, :, -n_patches:] if n_tokens_all > n_patches else raw_attn
                saliency_map_raw = cls_attn.mean(dim=1)
            ...
            if saliency_scores.shape[1] != grid_h * grid_w:
                side_dino = int(saliency_scores.shape[1] ** 0.5)
                scores_interp = F.interpolate(saliency_scores.reshape(-1, 1, side_dino, side_dino), size=(grid_h, grid_w), mode='bilinear')
                raw_map_interp = F.interpolate(saliency_map_raw.reshape(-1, 1, side_dino, side_dino), size=(grid_h, grid_w), mode='bilinear')
                saliency_map_for_at = raw_map_interp.flatten(1)
                mask_token, _ = self.get_saliency_guided_mask(scores_interp.flatten(1), mask_ratio=self.cfg.MASK_RATIO, mask_mode='hard')
            else:
            ...
            logits_student_masked, feats_student_masked, dino_cls_masked = raw_network(image_masked)
            logits_student_full, feats_student_full, dino_cls_full = raw_network(image_aug)

            tra_full_cls = self.tra_projector(dino_cls_full)
            logits_teacher = self.dino_classifier(tra_full_cls)
            ...
            z_student = F.normalize(self.predictor(self.projector(feats_student_full)), dim=1)
            with torch.no_grad():
                z_teacher_tra = F.normalize(self.dino_contrast_projector(tra_full_cls), dim=1)

            l_pos = torch.einsum('nc,nc->n', [z_student, z_teacher_tra]).unsqueeze(-1)
            l_queue = torch.einsum('nc,ck->nk', [z_student, self.queue.clone().detach().t()])

            logits_con = torch.cat([l_pos, l_queue], dim=1) / 0.07

            queue_labels = self.label_queue.clone().detach()  # (K,)
            mask_queue = (label.unsqueeze(1) == queue_labels.unsqueeze(0)).float()
            mask_queue = mask_queue * (queue_labels.unsqueeze(0) != -1).float()

            mask_con = torch.cat([torch.ones((z_student.shape[0], 1), device=curr_device), mask_queue], dim=1)

            log_prob = F.log_softmax(logits_con, dim=1)
            loss_contrast = - (mask_con * log_prob).sum(1) / mask_con.sum(1)
            loss_contrast = loss_contrast.mean()

            self._dequeue_and_enqueue(z_teacher_tra, label)

            tia_full_cls = self.tia_projector(dino_cls_full)
            loss_ortho = self.orthogonality_loss(tra_full_cls, tia_full_cls)

            with torch.no_grad():
                z_teacher_tia = F.normalize(self.dino_contrast_projector(tia_full_cls), dim=1)
            ...
            """
            ===================================================================
            [MICCAI Time Capsule: Advanced Idea 2 - Dense Patch-level Contrastive]
            We engineered a formidable pixel-level contrastive loss.
            It was mathematically beautiful but completely removed to restore 
            baseline convergence stability.
            ===================================================================
            """
            loss_dense = torch.tensor(0.0, device=curr_device)
            ...
            total_loss = loss_cls + lambda_distill * loss_logit_distill + lambda_contrast * loss_contrast + lambda_ortho * loss_ortho + lambda_fd_cls * loss_fd_cls + lambda_at * loss_at + lambda_neg * loss_neg_distill + lambda_dense * loss_dense
            total_loss = total_loss / accum_iter

        loss_dict_iter = {'loss': total_loss.item() * accum_iter, 'cls': loss_cls.item(), 'distill': loss_logit_distill.item(), 'contrast': loss_contrast.item(), 'ortho': loss_ortho.item(), 'at': loss_at.item(), 'neg': loss_neg_distill.item(), 'dense': loss_dense.item()}

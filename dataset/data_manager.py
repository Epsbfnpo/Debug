...
def get_dataset(args, cfg):
    ...
    train_dataset = GDRBench(root=cfg.DATASET.ROOT, source_domains=cfg.DATASET.SOURCE_DOMAINS, target_domains=cfg.DATASET.TARGET_DOMAINS, mode='train', trans_basic=train_ts, trans_fundus=tra_fundus, trans_mask=tra_mask)
    ...
    val_dataset = GDRBench(root=cfg.DATASET.ROOT, source_domains=cfg.DATASET.SOURCE_DOMAINS, target_domains=cfg.DATASET.TARGET_DOMAINS, mode='val', trans_basic=test_ts, trans_mask=tra_mask)
    ...
    test_dataset = GDRBench(root=cfg.DATASET.ROOT, source_domains=cfg.DATASET.SOURCE_DOMAINS, target_domains=cfg.DATASET.TARGET_DOMAINS, mode='test', trans_basic=test_ts, trans_mask=tra_mask)
...
def get_pre_FundusAug(cfg):
    ...
    tra_train_weak = transforms.Compose([transforms.Resize((size, size)), transforms.RandomCrop(re_size), transforms.RandomHorizontalFlip(), transforms.ToTensor(), normalize])

    tra_train_strong = transforms.Compose([transforms.Resize((size, size)), transforms.RandomCrop(re_size), transforms.RandomHorizontalFlip(), FundusAug.Compose([FundusAug.Sharpness(prob=aug_prob), FundusAug.Halo(re_size, prob=aug_prob), FundusAug.Hole(re_size, prob=aug_prob), FundusAug.Spot(re_size, prob=aug_prob), FundusAug.Blur(prob=aug_prob)]), transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1), transforms.ToTensor(), normalize])

    tra_test = transforms.Compose([transforms.Resize((size, size)), transforms.CenterCrop(re_size), transforms.ToTensor(), normalize])

    tra_mask = transforms.Compose([transforms.Resize((size, size)), transforms.CenterCrop(re_size), transforms.ToTensor()])
        ...
        try:
            if os.path.exists(cfg.MODEL.FPT.LPM_PATH):
                ...
                if hasattr(processor, 'image_mean'):
                    mean = processor.image_mean if isinstance(processor.image_mean, list) else [processor.image_mean] * 3
